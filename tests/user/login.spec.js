...
test("Login using a registered account and logout.", async ({ page }) => {
  ...
  const step2 = await page.locator("xpath=/html/body/div[2]/div[1]/div/div[3]/div[1]").innerText();

  await expect(step2).toContain("STEP 2");
  ...
  const step3 = await page.locator("xpath=/html/body/div[2]/div[1]/div/div[3]/div[1]").innerText();

  await expect(step3).toContain("STEP 3");
  ...
  const step4 = await page.locator("xpath=/html/body/div[2]/div[1]/div/div[3]/div[1]").innerText();

  await expect(step4).toContain("STEP 4");
...
test("Login using an unregistered account.", async ({ page }) => {
  ...
  await page.fill("#email", "preassess_regu@dswd.gov.ph");
