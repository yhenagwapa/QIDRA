...
test.describe.serial("Generate Ticket", () => {
  ...
  //   // If only 1 row exists, check if it's the "No Returnees" message row
  //   if (rowsCount === 1) {
  //       const messageCell = rows.first().locator('td');
  //       const text = await messageCell.innerText();

  //       if (text.includes('No Returnees found for yesterday')) {
  //           console.log('⚠️ No Returnees found — Test skipped.');
  //           test.skip();
  //       }
  //   }
  ...
  //       await rows.nth(0).locator('td').nth(4).getByRole('button', { name: /resume transaction/i }).click(),
  //       page.locator('#generateQueueBtn' ).click(),
  //     ]);
  //   // }
